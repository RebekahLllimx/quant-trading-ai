...
function makeGrid(top) {
  return { left: 55, right: 25, top: top || 40, bottom: 40 };
}
...
function drawSignals(rawData, result, stockName, stockCode) {
  ...
  const option = {
    title: { text: `${stockName} (${stockCode}) — 价格 · 均线 · 交易信号`, left: 10, top: 6,
      textStyle: { fontSize: 13, color: '#2d3436' } },
    tooltip: { trigger: 'axis' },
    legend: { data: ['收盘价', 'MA短', 'MA长'], top: 6, right: 10,
...
function drawEquity(rawData, result, metrics, stockName) {
  ...
  const option = {
    title: { text: `${stockName} — 资产曲线对比 (策略 vs 买入持有)`, left: 10, top: 6,
      textStyle: { fontSize: 12, color: '#2d3436' } },
    tooltip: { trigger: 'axis' },
    legend: { data: ['策略资产', '买入持有'], top: 6, right: 10,
      textStyle: { fontSize: 10, color: '#636e72' } },
    grid: makeGrid(46),
    xAxis: dateAxis(dates),
...
function drawDrawdown(rawData, result, metrics, stockName) {
  ...
  const option = {
    title: { text: `${stockName} — 回撤曲线 (Max DD: ${metrics ? metrics.mdd.toFixed(2) : '--'}%)`, left: 10, top: 6,
      textStyle: { fontSize: 12, color: '#2d3436' } },
    tooltip: {
      trigger: 'axis',
