...
最终报告见[`reports/Task6_Report.pdf`](../reports/Task6_Report.pdf)，其中包含主任务、附加题、代码、表格、图形与结论。可复现流水线读取`inputs/`中的规范化CSV输入。
...
- 选股策略：每季度买入预测收益排名前 30 只股票并等权持有，结果文件列出三个测试季度的完整股票代码。
- 基准：同季度样本股票等权平均收益率。
...
主任务输出包括季度预测、严格Top30与缓冲区组合持仓、换手、毛收益、成本后净收益和模型评价；增强实验补充EW Top30、PW Top30及验证期选定组合。附加题保存滚动预测概率、双阈值仓位、止损止盈事件和三类策略净值。`validate_results.py`逐项检查时间划分、组合恒等式、权重上限、成本公式、模型文件和Dashboard字段。Dashboard提供主任务与附加题的模型比较、季度表现、持仓、回撤和交易成本视图，另附纯前端CSV回归工具，便于使用相同字段结构检查新的数据文件。
