...
def main() -> None:
    ...
    error_outputs = [output for cell in code_cells for output in cell.get("outputs", []) if output.get("output_type") == "error"]
    add("notebook_executed_without_errors", all(cell.get("execution_count") is not None for cell in code_cells) and not error_outputs, f"code cells={len(code_cells)}, unexecuted={sum(cell.get('execution_count') is None for cell in code_cells)}, errors={len(error_outputs)}")
    markdown_text = "\n".join(cell.source for cell in notebook.cells if cell.cell_type == "markdown")
    forbidden_headings = ["tl;dr", "Context & Methods", "## Data", "## Results", "Takeaways"]
    add("notebook_chinese_assignment_headings", not any(item in markdown_text for item in forbidden_headings), f"forbidden hits={[item for item in forbidden_headings if item in markdown_text]}", severity="medium")
    add("notebook_no_name_line", "姓名" not in markdown_text and "Rebecca" not in markdown_text, "no name or name field in markdown", severity="medium")
    latest_codes = set(
        holdings[
            (holdings["model"] == strategy_model[0])
            & (holdings["portfolio"] == "strict_top30")
            & (holdings["Date"] == holdings["Date"].max())
        ]["Code"]
    ) if strategy_model else set()
    add(
        "notebook_latest_holdings_stated",
        len(latest_codes) == TOP_N and all(code in markdown_text for code in latest_codes),
        f"latest stated codes={sum(code in markdown_text for code in latest_codes)}/{len(latest_codes)}",
        severity="medium",
    )
    add("notebook_no_unicode_dashes", "—" not in markdown_text and "–" not in markdown_text, "em/en dash count=0" if "—" not in markdown_text and "–" not in markdown_text else "unicode dash found", severity="medium")
    language_audit_path = MAIN_METADATA_DIR / "language_audit.json"
    language_audit = pd.read_json(language_audit_path, typ="series") if language_audit_path.exists() else None
    language_score = language_audit["five_step_audit"]["4_five_dimension_score"]["total"] if language_audit is not None else 0
    language_passed = language_audit["five_step_audit"]["5_recheck"]["passed"] if language_audit is not None else False
    add("notebook_language_audit", bool(language_passed) and language_score >= 42, f"score={language_score}, target=42", severity="medium")
    add("notebook_sha256_recorded", notebook_path.exists() and notebook_path.stat().st_size > 40_000, f"sha256={sha256_file(notebook_path) if notebook_path.exists() else 'missing'}", severity="medium")
